				...
				if (dr[o.simk]) {
					...
					switch (o.state) {
						case 'init':
							...
							break;
						case 'star':
							$("#simker0"+s).find("div").eq(1).html('<i class="fa fa-play" style="cursor:pointer"></i>');
							$("#simker0"+s).find("div").eq(3).html('running&nbsp;&nbsp;'+o.speed+'x');
							break;
						case 'paus':
							$("#simker0"+s).find("div").eq(1).html('<i class="fa fa-pause" style="cursor:pointer"></i>');
...
$(document).ready(function(){
	...
	$("body").on("dblclick",".fa-play",function(){
			...
			$t.find("div").eq(1).html('<i class="fa fa-pause" style="cursor:pointer"></i>');
	...
	$("body").on("dblclick",".fa-pause",function(){
			...
			$t.find("div").eq(1).html('<i class="fa fa-play" style="cursor:pointer"></i>');
			$t.find("div").eq(3).html('running&nbsp;&nbsp;1x');
			dr[$t.attr("id").substring(7).replace(/_/,'.')] = 1;
	});
	$("body").on("long-press",".fa-play",function(){
