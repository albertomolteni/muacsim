...
(function($, window, document, undefined) {
  ...
  $.extend(nuContextMenu.prototype, {
    ...
    _contextMenu: function(event) {
      ...
      this._menu.css({
        'top' :                                 event.pageY  + 'px',
        'left': Math.min($("body").width()*0.75,event.pageX) + 'px'
      });
    ...
    _bindEvents: function() {
      ...
      if (this.items) {
        ...
        this.container.on(this.options.trigger, this.options.items,
                          $.proxy(this._contextMenu,
                                  this));
      } else {
        this.container.on(this.options.trigger, $.proxy(this._contextMenu,
                                                 this));
      }
